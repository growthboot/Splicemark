export default class TextEditor {
	edit(content, options) {
		if (options.mode === 'lines') {
			return this.#editLines(content, options);
		}

		if (options.mode === 'chars') {
			return this.#editChars(content, options);
		}
	...
	batch(content, options) {
		...
		if (options.mode === 'lines') {
			return this.#batchLines(content, splices);
		}

		if (options.mode === 'chars') {
			return this.#batchChars(content, splices);
		}

		throw new Error('batch mode must be lines or chars');
	}

	#batchLines(original, splices) {
